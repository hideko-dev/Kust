...
enum class KMaterial(data: Int, vararg legacy: String) {
    ...
    val legacy: Array<String> = legacy as Array<String>
    private val material: Material?
    init {
        var mat: Material? = null
        if ((this.isDuplicated) || (Material.getMaterial(this.name).also { mat = it }) == null) {
    ...
    constructor(vararg legacy: String?) : this(0, *legacy as Array<String>)
    private fun anyMatchLegacy(name: String): Boolean {
        for (i in legacy.indices.reversed()) {
    ...
    val id: Int get() {
        val material = this.material ?: return -1
        return try { material.id } catch (ignored: IllegalArgumentException) { -1 }
    }
    fun item(): ItemStack {
    ...
    fun isSimilar(item: ItemStack): Boolean {
        ...
        if (item.type != this.material()) return false
        if (this == SPLASH_POTION) {
            return item.durability == 16384.toShort()
        }
        return item.durability == data.toShort() || item.type.maxDurability > 0
    }

    private val isSupported: Boolean get() = this.material != null
    fun or(alternateMaterial: KMaterial?): KMaterial? {
    ...
    companion object {
        val VALUES: Array<KMaterial> = entries.toTypedArray()
        private val NAMES: MutableMap<String, KMaterial> = HashMap()
        private val NAME_CACHE: Cache<String, KMaterial> = CacheBuilder.newBuilder()
        ...
        private fun getIfPresent(name: String): Optional<KMaterial> {
            return Optional.ofNullable(NAMES[name])
        }
        private fun requestOldKMaterial(name: String, data: Byte): KMaterial? {
        ...
        private fun matchData(name: String): Optional<KMaterial> {
            val index = name.indexOf(':')
            if (index != -1) {
                val mat = format(name.substring(0, index))
                try {
                    val data = name.substring(index + 1).replace(" ", "").toInt().toByte()
                    return if (data in 0..<MAX_DATA_VALUE) matchDefined(mat, data) else matchDefined(mat, UNKNOWN_DATA_VALUE)
                } catch (ignored: NumberFormatException) {
                    return matchDefined(mat, UNKNOWN_DATA_VALUE)
                }
            }
            return Optional.empty()
        }
        fun match(name: String): Optional<KMaterial> {
            require(!(name.isEmpty())) { "Cannot match a material with null or empty material name" }
            val oldMatch = matchData(name)
            return if (oldMatch.isPresent) oldMatch else matchDefined(format(name), UNKNOWN_DATA_VALUE)
        }
        fun match(material: Material): KMaterial {
            Objects.requireNonNull(material, "Cannot match null material")
            return matchDefined(material.name, UNKNOWN_DATA_VALUE).orElseThrow { IllegalArgumentException("Unsupported material with no data value: " + material.name) }
        }
        fun match(item: ItemStack): KMaterial {
            Objects.requireNonNull(item, "Cannot match null ItemStack")
            var material = item.type.name
            val data = (if (material == "MAP" || item.type.maxDurability > 0) 0 else item.durability).toByte()
            if (item.hasItemMeta() && material == "MONSTER_EGG") material = item.type.name + "_SPAWN_EGG"
            if (material == "POTION") {
                val damage = item.durability.toInt()
                return if (((damage and 16384) > 0)) SPLASH_POTION else POTION
            }
            when (material) {
                "CACTUS_GREEN" -> return GREEN_DYE
                "ROSE_RED" -> return RED_DYE
                "DANDELION_YELLOW" -> return YELLOW_DYE
            }
            val result = matchDefined(material, data)
            if (result.isPresent) return result.get()
            throw IllegalArgumentException("Unsupported material from item: $material ($data)")
        }
        protected fun matchDefined(name: String, data: Byte): Optional<KMaterial> {
            var duplicated: Boolean? = null
            val isAMap = name.equals("MAP", ignoreCase = true)
            if ((!isAMap && data <= 0 && !(isDuplicated(name).also { duplicated = it }))) {
                val kmaterial = getIfPresent(name)
                if (kmaterial.isPresent) return kmaterial
            }
            val oldKMaterial = requestOldKMaterial(name, data) ?: return if ((data >= 0 && isAMap)) Optional.of(FILLED_MAP) else Optional.empty()
            val isPlural = oldKMaterial == CARROTS || oldKMaterial == POTATOES || oldKMaterial == BRICKS
            if (isPlural && (if (duplicated == null) isDuplicated(name) else duplicated!!)) return getIfPresent(name)
            return Optional.of(oldKMaterial)
        }
        protected fun format(name: String): String {
            val len = name.length
            ...
            for (i in 0 until len) {
                ...
                if (!appendUnderline && count != 0 && (ch == '-' || ch == ' ' || ch == '_') && chs[count] != '_') {
                    ...
                    var number = false
                    if ((ch in 'A'..'Z') || (ch in 'a'..'z') || ((ch in '0'..'9').also { number = it })) {
                        if (appendUnderline) {
                            chs[count++] = '_'
